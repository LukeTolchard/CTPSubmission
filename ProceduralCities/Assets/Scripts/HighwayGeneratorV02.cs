...
namespace CityGenerator {
    public class HighwayGeneratorV02 : BaseRoad {
        ...
        //public BitmapReader bitmapReader;
        ...
        protected override void Start() {
            ...
            /*
            bitmapReader = gameObject.AddComponent<BitmapReader>();
            if (!gameObject.GetComponent<BitmapReader>())
            {
                Debug.Log("BitmapReader script not attached correctly");
            }*/
        }
        ...
        protected override void Update() {
            if (curLenght < maxLenght) {
                if (this.transform.position.x > 0 && this.transform.position.x < settings.populationMap.width && this.transform.position.z > 0 && this.transform.position.z < settings.populationMap.height) {
                    ...
                    {
                        Debug.Log("Position isn't assigned correctly");
                    }
                    
                    //Attempt to call the height correction to suitably adjust height on a terrain after generation

                    /*int heightCorrection = 0;
                    heightCorrection = bitmapReader.CorrectHeight((int)this.transform.position.x, (int)this.transform.position.z);
                    Debug.Log(heightCorrection);*/

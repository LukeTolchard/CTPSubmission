    ...
    public class StreetGeneratorV01 : BaseRoad {
        ...
        protected override Vector3 PopulationConstraints(List<int> x, List<int> z, List<float> y) {
            ...
            for (int i = 0; i < x.Count; i++) {
                ...
                if (heighestPopulation < highest) {
                    heighestPopPos = new Vector3(x[i], y[i], z[i]);
                    highest = heighestPopulation;
                }
            }
            if(minimalPopulation < highest) {
                DestroyGenerator();
            }
        ...
        void GenerateBuildingMesh(GameObject streetBuilding, Vector3 start, Vector3 end, bool leftStreet, bool newGen, bool corner) {
            float offset = 0;
            float roadOffset = 5.0f;
            float cornerOrEndDecrease = 1;
            float streetLength = (Vector2.Distance(new Vector2(start.x, start.z), new Vector2(end.x, end.z)));

            if (corner && newGen) {              
                cornerOrEndDecrease = (streetLength / 8);
            }else if (corner) {
                cornerOrEndDecrease = (streetLength / 4);
            }
            ...
            for (int i = 0; i < divider; i++) {
                sum = Random.Range((streetLength / divider), streetLength);
                streetLength -= sum;
                divisions.Add(sum);
            }
            ...
            foreach (float d in divisions) {
                ...
                //Pointy Towers
                if(buildingType == 3 && d > maxBuildingSize/2 && pointTowerThreshhold < highest) {
                    float size = d;
                    float height = Random.Range((laserDistance) + highest, (laserDistance + highest) * 4);
                ...
                //House
                else if (homeThreshold < highest && d > maxBuildingSize/2) {
                    float size = (d/2.5f);
                    float height = Random.Range((laserDistance / 2) + highest, laserDistance + highest);
                ...
                //Tower Block
                    ...
                    float size = d;
                    float height = Random.Range((laserDistance) + highest, (laserDistance + highest) * 3);
        ...
        //Can now detect if is a new generator or new generator has been built by this Generator
